...
services_utils = function (session, rest) {
    ...
    rest.post('/v1/boards/:board/services/:service/action', function (req, res) {
        ...
        var service = req.params.service;  //e.g.: ssh | osjs
        ...
        board_utility.checkBoardAvailable(board_id, res, function (available){
            ...
            if(available.result == "SUCCESS"){
                ...
                if (service_action == "enable" || service_action == "disable" || service_action == "restore") {
                    ...
                    var board_label = available.b_label;
                    ...
                    board_utility.checkRequired(ApiRequired, function (check) {
                        ...
                        if (check.result == "ERROR") {
                            ...
                            services_utils.manageService(board_id, service, service_action, res, null);
                        }
        ...
        if (service_action == "enable" || service_action == "disable" || service_action == "restore") {
            ...
            board_utility.checkRequired(ApiRequired, function (check) {
                ...
                if (check.result == "ERROR") {
                    ...
                    var subject = "service action: "+ service_action + " " +service;
    ...
    //GET boards list where a service is exposed
    /**
     * @swagger
     * /v1/services/{service}/boards:
     *   get:
     *     tags:
     *       - Services
     *     description: Get IoTronic boards list where a service is exposed
     *     summary: get boards list where a service is exposed
     *     parameters:
     *      - in: path
     *        name: service
     *        required: true
     ...
     *     responses:
     *       200:
     *          description: List of boards that expose a service
     *          properties:
     *              result:
     *                  type: string
     *                  description: "SUCCESS"
     ...
     *         description: "API specific error message."
     */
    rest.get('/v1/services/:service/boards', function (req, res) {

        logger.info("[API] - Board list per service - " + Object.keys( req.route.methods ) + " - " + req.route.path + " - " + req.IotronicUser);
        ...
        db.getBoardsPerService(service, function (data) {
    ...
    //GET boards list where a service is exposed in a project
    /**
     * @swagger
     * /v1/projects/{project}/services/{service}/boards:
     *   get:
     *     tags:
     *       - Services
     *     description: Get IoTronic boards list where a service is exposed in a project
     *     summary: get boards list where a service is exposed in a project
     *     parameters:
     *      - in: path
     *        name: project
     *        required: true
     ...
     *         description: "API specific error message."
     */
    rest.get('/v1/projects/:project/services/:service/boards', function (req, res) {
        ...
        db.getProjectBoardsPerService(project, service, function (data) {
                    ...
                    db.getActiveService(service_id, board_id, function (data_p) {
                        ...
                        if (data_p.result == "ERROR") {
                            ...
                            if (data_p.message.length != 0){
                                                ...
                                                if (check_result.result == "ERROR") {
                                                    ...
                                                    if(res != false){
                                                        ...
                                                        if(request_id != null)
                                                            ...
                                                            logger.info("[SYSTEM] --> " + service_result.message);
                                                    }
                                                    ...
                                                    response.message = service_result.message;
                                                    response.logs = service_msg;
                                                    response.result = "SUCCESS";
                                ...
                                response.result = "WARNING";
                                response.message = "Service '" + service_name + "' is not enabled!";
                    ...
                    db.getActiveService(service_id, board_id, function (data_p) {
                        ...
                        if (data_p.result == "ERROR") {
                            ...
                            if (data_p.message.length != 0) {
                                ...
                                console.log("PIIIIIIIIIIIIIIID: " +db_tunnel_pid)
                                        ...
                                        if (service_result.result === "WARNING") {
                                            ...
                                            if (service_result.result === "SUCCESS") {
                                                ...
                                                if (service_result.message.status === "INACTIVE") {
                                                    ...
                                                    logger.warn("[SERVICE] --> Service '" + service_name + "' of board '" + board_id + "' is in wrong state.. Updating...");
                    ...
                    db.getActiveService(service_id, board_id, function (data_p) {
                        ...
                        if (data_p.result == "ERROR") {
                            ...
                            if (data_p.message[0] == undefined) {
                                ...
                                response.message = "Service '" + service_name + "' not exposed!";
                                        ...
                                        if (service_result.result === "SUCCESS") {
                                            ...
                                            db.removeTunnel(service_id, board_id, function (check_result) {
                                                ...
                                                if (check_result.result == "ERROR") {
                                                    ...
                                                    //response.message = "Service " + service_name + " disabled on board " + board_id + " (public port = "+publicPort+")";
                                                    response.message = "Service '" + service_name + "' disabled on port "+publicPort;
    ...
    db.getBoardServices(board_id, function (data) {
        ...
        if (data.result == "ERROR") {
            ...
            if(data.message.length == 0){
                ...
                logger.info("[SERVICE] - Reload board services tunnels:\n"+ JSON.stringify(data.message, null, "\t"));
...
function newPort(callback) {
    ...
    var all_ports = range(wstun_l_port, wstun_h_port);

    var free = [];
    var busy_list = [];

    db.getBusyPorts(function (busy) {

        if (busy.result == "ERROR") {

            logger.error("[SERVICE] --> DB getBusyPorts error: " + busy.message);
            callback(busy)
            ...
            if (busy.message.length == 0){
                ...
                free = all_ports;
                logger.debug("[SERVICE] --> Free ports (all): " + free);
                var rand_selected_port = free[Math.floor(Math.random()*free.length)];

                response.message = rand_selected_port;
                response.result = "SUCCESS";
                callback(response);
                ...
                for(var key=0; key < busy.message.length; key++) {

                    (function(key) {

                        busy_list.push(busy.message[key].public_port);

                        if (key == busy.message.length - 1) {

                            logger.debug("[SERVICE] --> Busy ports: " + busy_list);

                            if(busy.message.length  < n_ports  ) {

                                free = all_ports.filter(n => !busy_list.includes(n))

                                logger.debug("[SERVICE] --> Free ports: " + free);
