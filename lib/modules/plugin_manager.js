        ...
        if(plugin_name.indexOf(' ') >= 0){
            ...
            board_utility.checkRequired(ApiRequired, function (check){
                ...
                if(check.result == "ERROR"){
                    ...
                    //console.log("ZIP bundle recived");
                    logger.debug("[PLUGIN] --> Plugin bundle analysis started:");

                    buffer = new Buffer(code, 'base64'); //get from DB and create a Buffer
                    logger.debug("[PLUGIN] - Buffered object:");
                    logger.debug(buffer);
                    ...
                    fs.open("/tmp/bundle_"+plugin_name+".zip", 'w', function(error, fd) {
                        ...
                        if (error) {
                            response.message = "WARNING: error opening plugin zip bundle : "+JSON.stringify(error);
                            response.result = "ERROR";
                            logger.warn("[PLUGIN] --> " + response.message);
                            res.status(500).send(response);
                        }
                            ...
                            fs.write(fd, buffer, 0, buffer.length, null, function(error) {
                                ...
                                if (error) {
                                    response.message = "WARNING: error creating plugin zip bundle : "+JSON.stringify(error);
                                    response.result = "ERROR";
                                    logger.warn("[PLUGIN] --> " + response.message);
                                    res.status(500).send(response);
                                }
                                    ...
                                    logger.debug("[PLUGIN] - Zip file created");

                                    fs.close(fd, function() {

                                        logger.debug("[PLUGIN] - Unzipping plugin bundle.");

                                        // unzip the plugin bundle in /var/lib/iotronic/plugins/<NAME-PLUGIN>
                                        compressing.zip.uncompress('/tmp/bundle_'+plugin_name+'.zip', '/tmp/'+plugin_name+'/').then(
                                            function(){
                                                ...
                                                hashElement('/tmp/'+plugin_name+'/source/', options).then(
                                                    ...
                                                    function(hash) {
                                                        ...
                                                        plugin_utils.createPlugin(plugin_name, plugincategory, parameters, code, version, type, description, checksum, res);
                                            ...
                                            function (error) {
                                                response.message = "WARNING: unzip error: "+JSON.stringify(error);
                                                response.result = "ERROR";
                                                logger.warn("[PLUGIN] --> " + response.message);
                                                res.status(500).send(response);
                                            }
...
plugin_utils.prototype.updateStatus = function (args) {
    ...
    db.getPluginFromVersion(plugin_name, plugin_version, function (data) {
        ...
        if(data.result=="ERROR"){
            ...
            db.updatePluginStatus(board_id, plugin_id, plugin_status, function (data) {
...
plugin_utils.prototype.createPlugin = function (name, category, parameters, code, version, type, description, checksum, res) {
                    ...
                    db.checkPluginExists(name, version, category, type_id, function (data) {
                        ...
                        if (data.result == "ERROR") {
                            ...
                            if (data.message[0] === undefined) {
                                ...
                                db.insertCreatedPlugin(name, category, parameters, code, version, type_id, description, checksum, function (result_db) {
                                                                        ...
                                                                        session_wamp.call('s4t.'+ board + '.plugin.run', [plugin_name, parameters, plugin_checksum]).then(
                                                                            ...
                                                                            function (rpc_response) {
                                                                                ...
                                                                                if (rpc_response.result == "ERROR") {
                                                                                    ...
                                                                                    db.updatePluginStatus(board, plugin, "started", function (out) {
