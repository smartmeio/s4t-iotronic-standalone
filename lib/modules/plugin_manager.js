...
plugin_utils = function (session, rest) {
     ...
     *                          iotronic_dep:
     *                              type: string
     *                              description: "Flag ('true' | 'false') used to specify if plugin workflow depends on Iotronic information"
     *                          tag:
     *                              type: string
     ...
     *                          packaging:
     *                              type: string
     *                              description: "Plugin packaging type ('single' | 'bundle')"
     *                          updated_at:
     *                              type: string
     *                              description: "Timestamp of the latest plugin change"
     *                          created_at:
    ...
    rest.post('/v1/plugins/', function (req, res) {
        ...
        if(plugin_name.indexOf(' ') >= 0){

            response.message = "WARNING: please specify plugin name without white spaces!";
            response.result = "ERROR";
            ...
            if (req.body.iotronic_dep != undefined)
                var iotronic_dep = req.body.iotronic_dep;
            else
                var iotronic_dep = "false";

            if (req.body.packaging != undefined)
                var packaging = req.body.packaging;
            else
                var packaging = "single";


            var ApiRequired = {"plugin_name":plugin_name, "plugin_category":plugincategory, "parameters":parameters, "code":code, "version":version, "type":type, "iotronic_dep":iotronic_dep, "packaging":packaging };
            ...
            board_utility.checkRequired(ApiRequired, function (check){
                ...
                if(check.result == "ERROR"){
                    ...
                    plugin_utils.createPlugin(plugin_name, plugincategory, parameters, code, version, type, description, iotronic_dep, packaging, res);
     ...
     *                  on_maintenance:
     *                      type: string
     *                      description: "Flag to specify if the plugin can be start in maintenance device state:\n- possible values: 'true' or 'false'"
     *                  force:
     *                      type: string
    ...
    rest.put('/v1/boards/:board/plugins', function (req, res) {
        ...
        board_utility.checkBoardAvailable(board, res, function (available){
            ...
            if(available.result == "SUCCESS"){
                ...
                var on_maintenance = req.body.on_maintenance;
                ...
                if ( (onboot != "true" && onboot != "false") || (force != "true" && force != "false") || (on_maintenance != "true" && on_maintenance != "false") ){

                    response.message = "Check 'onboot', 'force', 'on_maintenance' parameters [ String: 'true' | 'false' ]";
                    response.result = "ERROR";
                    ...
                    board_utility.checkRequired(ApiRequired, function (check){
                        ...
                        if(check.result == "ERROR"){
                            ...
                            plugin_utils.injectPlugin(board, null, [plugin, onboot, force, on_maintenance, res]);
                        ...
                        if (data.result == "ERROR") {
                            ...
                            if (data.message[0] === undefined) {
                                ...
                                if (pluginTag == "released") {
                                    ...
                                    request_utils.batchRequest(project, subject, res, plugin_utils.injectPlugin, [plugin, onboot, force, on_maintenance, false]);

                                }
                                else{

                                    response.message = "Plugin not released!";
                                    response.result = "ERROR";
                                    logger.warn("[PLUGIN] --> " + response.message);
                                    res.status(500).send(response);
        ...
        if(name.indexOf(' ') >= 0){
            ...
            board_utility.checkRequired(ApiRequired, function (check){
                ...
                if(check.result == "ERROR"){
                    ...
                    plugin_utils.updatePlugin(plugin, name, defaults, code, description, version, tag_id, iotronic_dep, res);
    ...
    //Inject plugins configuration
    /**
     * @swagger
     * /v1/boards/{board}/plugins_conf:
     *   put:
     *     tags:
     *       - Plugins
     *     summary: inject plugins configuration to the board
     *     description: Inject plugins configuration to the board
     *     produces:
     *       - application/json
     *     parameters:
     *      - in: path
     ...
     *         description: "API specific error message."
     */
    rest.put('/v1/boards/:board/plugins_conf', function (req, res) {

        logger.info("[API] - Inject plugins conf called - " + Object.keys( req.route.methods ) + " - " + req.route.path + " - " + req.IotronicUser);

        var board = req.params.board;

        plugin_utils.injectConf(board, null, [res]);


    });


    //BATCH: Inject plugins configuration
    /**
     * @swagger
     * /v1/projects/{project}/plugins_conf:
     *   put:
     *     tags:
     *       - Plugins
     *     description: Injects plugins configuration in the project's boards
     *     summary: inject plugins configuration in the project's boards
     *     parameters:
     *      - in: path
     *        name: project
     *        required: true
     ...
     *         description: "API specific error message."
     */
    rest.put('/v1/projects/:project/plugins_conf', function (req, res) {

        logger.info("[API] - BATCH Inject plugins configuration - " + Object.keys( req.route.methods ) + " - " + req.route.path + " - " + req.IotronicUser);

        var project = req.params.project;

        var subject = "plugins configuration inject";

        request_utils.batchRequest(project, subject, res, plugin_utils.injectConf, [false]);

    });


    //GET device plugins configuration
    /**
     * @swagger
     * /v1/boards/{board}/plugins_conf:
     *   get:
     *     tags:
     *       - Plugins
     *     description: Return device plugins config file
     *     summary: get plugins config file
     *     parameters:
     *      - in: path
     *        name: board
     *        required: true
     ...
     *                  description: "SUCCESS"
     *              message:
     *                  title: JSON plugins config file
     *                  type: object
     *       403:
     *         description: "Wrong, expired or not specified token in request header."
     *       500:
     *         description: "API specific error message."
     */
    rest.get('/v1/boards/:board/plugins_conf', function (req, res) {

        logger.info("[API] - Get plugins.json - " + Object.keys( req.route.methods ) + " - " + req.route.path + " - " + req.IotronicUser);
        ...
        plugin_utils.getPluginsConfs(board).then(
            ...
            function (conf) {
                ...
                if (conf.result == "ERROR") {
                    ...
                    response.message = conf.message;
                    response.result = "SUCCESS";
                    logger.info("[SYSTEM] --> plugins.json returned.");
                    res.status(200).send(response);
...
plugin_utils.prototype.updatePlugin = function (plugin, name, defaults, code, description, version, tag_id, iotronic_dep, res) {
    ...
    db.getPlugin(plugin, function (data) {
        ...
        if (data.result == "ERROR") {
            ...
            if (data.message[0] === undefined) {
                ...
                db.updatePlugin(plugin_id, name, version, tag_id, code, defaults, description, iotronic_dep, function (result_db) {
...
plugin_utils.prototype.invalidPlugin = function (args) {
    ...
    db.getPluginFromVersion(plugin_name, plugin_version, function (data) {
        ...
        if(data.result=="ERROR"){
            ...
            db.updatePluginStatus(board_id, plugin_id, "invalid", null,null, function (data) {
...
plugin_utils.prototype.updateStatus = function (args) {
    ...
    var plugin_name = args[1];
    var plugin_version = args[2];
    var plugin_status = args[3];

    logger.debug("[PLUGIN] - Updating status for plugin '"+plugin_name+"' of board '" + board_id +"'!");
    ...
    db.getPluginFromVersion(plugin_name, plugin_version, function (data) {
        ...
        if(data.result=="ERROR"){
            ...
            db.updatePluginStatus(board_id, plugin_id, plugin_status, null,null, function (data) {
                ...
                if(data.result=="ERROR"){
                    ...
                    response.message = "Iotronic updated the status for the '"+plugin_name+"' plugin.";
                    response.result = "SUCCESS";
                    d.resolve(response);
...
plugin_utils.prototype.createPlugin = function (name, category, parameters, code, version, type, description, iotronic_dep, packaging, res) {
                    ...
                    db.checkPluginExists(name, version, category, type_id, function (data) {
                        ...
                        if (data.result == "ERROR") {
                            ...
                            if (data.message[0] === undefined) {
                                ...
                                db.insertCreatedPlugin(name, category, parameters, code, version, type_id, description, iotronic_dep, packaging, function (result_db) {
                            ...
                            if(pluginChecksum === checksum){
                                ...
                                db.getInjectedPlugin(pluginId, board, function (data) {
                                    ...
                                    if (data.result == "ERROR") {
                                        ...
                                        // Plugin already injected --> NO ACTION ON BOARD
                                        if(data.message.length != 0 && (force == false || force == "false")){
                                            ...
                                            if(res != false)
                                                logger.debug("[PLUGIN] --> Calling injectplugin RPC with name = '" + pluginName + "' onboot = '" + autostart + "' - force = '" + force + "' - on_maintenance = '" + on_maintenance + "'  code: \n" + pluginCode +
                                                    "\n\nwith the following default parameters:\n"+pluginParameters);

                                            //Now I can perform the RPC call to inject the plugin
                                            session_wamp.call('s4t.'+ board + '.plugin.inject', [JSON.stringify(pluginBundle) , autostart, force, on_maintenance]).then(
                                                ...
                                                function (rpc_response) {
                                                    ...
                                                    if(rpc_response.result == "ERROR"){
                                                        ...
                                                        if( data.message.length != 0 && (force == true || force == "true") ){
                                                            ...
                                                            db.updatePluginStatus(board, pluginId, "injected", autostart, on_maintenance, function (out) {
                                                            ...
                                                            db.deleteInjectedPlugin(board, pluginName, function (data_p) {
                                                                ...
                                                                if (data_p.result == "ERROR") {
                                                                    ...
                                                                    db.insertInjectedPlugin(board, pluginId, pluginName, autostart, on_maintenance, function (out) {
...
plugin_utils.prototype.managePlugins = function (board, request_id, args) {
    ...
    board_utility.checkBoardAvailable(board, res, function (available) {
        ...
        if (available.result == "SUCCESS") {
            ...
            //logger.debug("DEVICE STATE: " + available.state)
            var device_state = available.state;
                        ...
                        db.getInjectedPlugin(plugin, board, function (data) {
                            ...
                            if(data.result=="ERROR"){
                                ...
                                if(data.message.length == 0){
                                    ...
                                    var start_on_maintenance = data.message[0].on_maintenance;
                                    logger.debug("[PLUGIN] --> Device state evaluation: " + device_state + " - " + start_on_maintenance);

                                    try {


                                        // check plugin parameters and insert it in DB
                                        if(plugin_operation != "kill" && plugin_operation != "restart"){

                                            if(device_state != "maintenance" && start_on_maintenance != "false") {
                                        ...
                                        switch (plugin_operation) {
                                            ...
                                            case 'run':
                                                ...
                                                if(device_state == "maintenance" && start_on_maintenance == "false"){
                                                    ...
                                                    if(res != false){
                                                        response.result = "WARNING";
                                                        response.message = "This ASYNC plugin is not allowed to be started when device is on maintenace!";
                                                        logger.warn("[PLUGIN] --> " + response.message);
                                                        res.status(500).send(response);
                                                    }else{
                                                        request_utils.updateResult(request_id, board, response.result, response.message);
                                                                            ...
                                                                            session_wamp.call('s4t.'+ board + '.plugin.run', [plugin_name, parameters, plugin_checksum]).then(
                                                                                ...
                                                                                function (rpc_response) {
                                                                                    ...
                                                                                    if (rpc_response.result == "ERROR") {
                                                                                        ...
                                                                                        db.updatePluginStatus(board, plugin, "failed", null, start_on_maintenance, function (out) {

                                                                                            if(res != false){
                                                                                                logger.debug("[PLUGIN] ----> " + out.message);
                                                                                                logger.warn("[PLUGIN] --> Starting plugin error for '" + plugin_name + "': " + JSON.stringify(response.message, null, "\t"));
                                                                                                res.status(500).send(response);
                                                                                            }else{
                                                                                                request_utils.updateResult(request_id, board, response.result, response.message);
                                                                                            }
                                                                                        ...
                                                                                        db.updatePluginStatus(board, plugin, "started", null, start_on_maintenance, function (out) {
                                            ...
                                            case 'call':
                                                ...
                                                if(device_state == "maintenance" && start_on_maintenance == "false"){
                                                    if(res != false){
                                                        response.result = "WARNING";
                                                        response.message = "This SYNC plugin is not allowed to be started when device is on maintenace!";
                                                        logger.warn("[PLUGIN] --> " + response.message);
                                                        res.status(500).send(response);
                                                    }else{
                                                        request_utils.updateResult(request_id, board, response.result, response.message);
                                                                        ...
                                                                        session_wamp.call('s4t.'+ board + '.plugin.call', [plugin_name, parameters, plugin_checksum], {}, {receive_progress: true}).then(
                                                                            ...
                                                                            function (rpc_response) {
                                                                                ...
                                                                                if (rpc_response.result == "ERROR") {
                                                                                    ...
                                                                                    db.updatePluginStatus(board, plugin, "executed", null, start_on_maintenance, function (out) {

                                                                                        if(res != false){
                                                                                            logger.debug("[PLUGIN] ----> " + out.message);
                                                                                            logger.debug("[PLUGIN] --> Executing result for '" + plugin_name + "' plugin:\n" + JSON.stringify(rpc_response, null, "\t"));
                                                                                            res.status(200).send(response);
                                                                                        }else{
                                                                                            request_utils.updateResult(request_id, board, response.result, response.message);
                                                                                        }
                                                ...
                                                session_wamp.call('s4t.'+ board + '.plugin.kill', [plugin_name]).then(
                                                    ...
                                                    function (rpc_response) {
                                                        ...
                                                        if (rpc_response.result == "ERROR") {
                                                            ...
                                                            db.updatePluginStatus(board, plugin, "killed", null, null,function (out) {
                                                ...
                                                session_wamp.call('s4t.'+ board + '.plugin.restart', [plugin_name, plugin_checksum]).then(
                                                    ...
                                                    function (rpc_response) {
                                                        ...
                                                        if (rpc_response.result == "ERROR") {
                                                            ...
                                                            db.updatePluginStatus(board, plugin, "failed", null, null,function (out) {
                                                            ...
                                                            db.updatePluginStatus(board, plugin, "restarted", null, null,function (out) {
...
var createPluginConf = function (board_id, callback) {
    ...
    db.BoardPlugins(board_id, function (data) {

        if (data.result == "ERROR") {

            response.message = "Error getting board plugins: " + data.message;
            response.result = "ERROR";
            logger.error("[SYSTEM] --> " + response.message);
            callback(response);
            ...
            //var confs = JSON.parse(fs.readFileSync(IotronicHome+"/board_plugins_template.json", 'utf8'));
            var confs = {
                "plugins":{
            ...
            for (var i = 0; i < data.message.length; i++) {
                ...
                (function (i) {
                    ...
                    //logger.error(data.message[i].name);

                    confs.plugins[data.message[i].name] = {
                        "category": data.message[i].category,
                        "status": "off",
                        "pid": "",
                        "type": data.message[i].type,
                        "version": data.message[i].version,
                        "autostart": data.message[i].autostart,
                        "on_maintenance": data.message[i].on_maintenance,
                        "iotronic_dep": data.message[i].iotronic_dep

                    };

                    if (i == data.message.length -1 ){

                        response.message = confs;
                        response.result = "SUCCESS";
                        //logger.debug(confs)
                        callback(response);
                    }
...
plugin_utils.prototype.getPluginsConfs = function (board_id) {
    ...
    logger.debug("[PLUGIN] - Getting plugins configuration for board '" + board_id + "'");

    createPluginConf(board_id, function (conf) {
...
plugin_utils.prototype.injectConf = function (board_id, request_id, args){

    logger.info("[SYSTEM] - Injecting plugins configuration into board '" + board_id + "'...");
    ...
    board_utility.checkBoardAvailable(board_id, res, function (available) {
        ...
        if (available.result == "SUCCESS") {
            ...
            plugin_utils.getPluginsConfs(board_id).then(
                ...
                function (conf) {
                    ...
                    if (conf.result == "ERROR") {
                        ...
                        session_wamp.call('s4t.'+ board_id + '.plugin.updateConf', [conf]).then(

                            function (rpc_response) {

                                if(rpc_response.result == "ERROR"){

                                    response.message = "INJECT plugin configuration into board " + board_id + " failed: "+rpc_response.message;
                                    response.result = "ERROR";
                                    if(res != false){
                                        logger.error("[SYSTEM] --> " + response.message);
                                        res.status(500).send(response);
                                    ...
                                    response.message = "Plugin configuration injected into the board '" + board_id + "'";
                                    response.result = "SUCCESS";
                                    if(res != false){
...
plugin_utils.prototype.exportCommands = function (session){
    ...
    session.register('s4t.iotronic.plugin.updateStatus', this.updateStatus);
    logger.debug('[WAMP-EXPORTS] --> s4t.iotronic.updateStatus');

    session.register('s4t.iotronic.plugin.getPLuginsConfs', this.getPluginsConfs);
    logger.debug('[WAMP-EXPORTS] --> s4t.iotronic.getPluginsConfs');
