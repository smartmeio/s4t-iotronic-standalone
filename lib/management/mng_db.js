                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    (function (i) {
                        ...
                        try{
                            ...
                            response.message = "Error parsing 'extra' field in BoardsList: " + err;
                            response.result = "ERROR";
                            logger.error("[SYSTEM] --> [Device " + result[i]["board_id"] +  "] - " + response.message);
                            result[i]["extra"] = "WRONG JSON FIELD: " + result[i]["extra"]
...
db_utils.prototype.updateDeviceExtraInfo = function (board_id, extra, callback) {
    ...
    connection.query("UPDATE boards SET extra=" + mysql.escape(extra) + " WHERE board_id=" + mysql.escape(board_id), function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
            ...
            response.message = "device extra info updated!";
            response.result = "SUCCESS";
            disconn(connection);
            callback(response);
        }
        ...
        if (err_info != null) {
            ...
            connection.query("SELECT altitude, longitude, latitude, timestamp FROM coordinates WHERE coordinates.board_id = " + mysql.escape(board) + " ORDER BY timestamp DESC LIMIT 1", function (err_coord, coord) {
                ...
                if (err_coord != null) {
                    ...
                    try{
                        if(result_info[0]["extra"] != "" && result_info[0]["extra"] != undefined)
                            result_info[0]["extra"] = JSON.parse(result_info[0]["extra"]);
                    }
                    catch (err) {
                        response.message = "Error parsing 'extra' field in BoardInfo: " + err;
                        response.result = "ERROR";
                        logger.error("[SYSTEM] --> [Device " + result_info[0]["board_id"] +  "] - " + response.message);

                        result_info[0]["extra"] = "WRONG JSON FIELD: " + result_info[0]["extra"]
                    }

                    connection.query("SELECT plugins.name, plugins.version, plugins.id, plugins.category, plugins.iotronic_dep, plugin_types.type, plugins_injected.state, plugins_injected.parameters, plugins_injected.autostart, plugins_injected.on_maintenance, plugins_injected.latest_change  FROM plugins_injected, plugins, plugin_types WHERE plugins_injected.board_id = " + mysql.escape(board) + " AND plugins_injected.plugin_id = plugins.id AND plugin_types.id = plugins.type_id", function (err_plugin, result_plugin) {
...
//Function to update the LR registration state
db_utils.prototype.changeBoardState = function (board_id, state, callback) {
    ...
    //var current_datetime = new Date(); var formatted_date = current_datetime.getFullYear() + "-" + (current_datetime.getMonth() + 1).toString().padStart(2, "0") + "-" + current_datetime.getDate().toString().padStart(2, "0") + " " + current_datetime.getHours().toString().padStart(2, "0") + ":" + current_datetime.getMinutes().toString().padStart(2, "0") + ":" + current_datetime.getSeconds().toString().padStart(2, "0")

    var state_time = new Date().toISOString().replace(/\..+/, '');

    connection.query("UPDATE boards SET state=" + mysql.escape(state) + ",  state_time=" + mysql.escape(state_time) + " WHERE board_id=" + mysql.escape(board_id), function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
            ...
            response.message = "board state updated to '"+state+"'";
            response.result = "SUCCESS";
            disconn(connection);
            callback(response);
        }
...
//Function to force device connection status
db_utils.prototype.forceConnStatus = function (board_id, status, callback) {
    ...
    connection.query("UPDATE boards SET status=" + mysql.escape(status) + ",  conn_time=" + mysql.escape(conn_time) + " WHERE board_id=" + mysql.escape(board_id), function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
            ...
            response.message = "connection status forced to '"+status+"'";
            response.result = "SUCCESS";
            disconn(connection);
            callback(response);
        }
...
db_utils.prototype.changeBoardStateOnUpdate = function (board_id, state, lr_version, callback) {
...
db_utils.prototype.insertCreatedPlugin = function (name, category, parameters, code, version, type, description, iotronic_dep, packaging, callback) {
    ...
    connection.query("INSERT INTO plugins (name, category, version, checksum, type_id, tag_id, code, defaults, description, iotronic_dep, packaging, created_at) VALUES (" +
        mysql.escape(name) + " , " +
        ...
        mysql.escape(description) + " , " +
        mysql.escape(iotronic_dep) + " , " +
        mysql.escape(packaging) + " , " +
        "NOW())",
...
db_utils.prototype.getPlugin = function (plugin, callback) {
    ...
    var query = "SELECT plugins.id, plugins.name, plugins.category, plugins.version, plugins.checksum, plugins.code, plugins.defaults, plugins.checksum, plugin_tags.tag, plugin_tags.id as tag_id, plugin_types.type, plugins.type_id, " +
        "plugins.created_at, plugins.updated_at, plugins.description, plugins.iotronic_dep, plugins.packaging " +
...
//Function to update the plugin status of a specific board
db_utils.prototype.updatePluginStatus = function (board, plugin_id, status, autostart, on_maintenance, callback) {
    ...
    var head_query = "UPDATE plugins_injected SET state = " + mysql.escape(status) + ",";
    var end_query = " latest_change=NOW() WHERE board_id=" + mysql.escape(board) + " AND plugin_id=" + mysql.escape(plugin_id);

    if(autostart != null)
        var head_query = head_query + " autostart = " + mysql.escape(autostart) + ",";

    if(on_maintenance != null)
        var head_query = head_query + " on_maintenance = " + mysql.escape(on_maintenance) + ",";

    var query = head_query + end_query;

    logger.debug(query)
    /*
    if(autostart==null)
        var query = "UPDATE plugins_injected SET state = " + mysql.escape(status) + ", latest_change=NOW() WHERE board_id=" + mysql.escape(board) + " AND plugin_id=" + mysql.escape(plugin_id);
    else
        var query = "UPDATE plugins_injected SET state = " + mysql.escape(status) + ", autostart = " + mysql.escape(autostart) + ", latest_change=NOW() WHERE board_id=" + mysql.escape(board) + " AND plugin_id=" + mysql.escape(plugin_id);
    */
...
db_utils.prototype.BoardPlugins = function (board, callback) {
    ...
    connection.query("SELECT plugins.name, plugins.version, plugins.id, plugins.description, plugins.category, plugins.iotronic_dep, plugins_injected.state, plugin_tags.tag, plugin_types.type, plugins_injected.autostart, plugins_injected.on_maintenance, plugins_injected.latest_change, plugins_injected.parameters FROM plugins_injected, plugins, plugin_tags, plugin_types WHERE " +
...
db_utils.prototype.insertInjectedPlugin = function (board, plugin_id, plugin_name, autostart, on_maintenance, callback) {
    ...
    connection.query("INSERT INTO plugins_injected (board_id, plugin_id, plugin_name, state, autostart, on_maintenance) VALUES (" + mysql.escape(board) + "," + mysql.escape(plugin_id) + "," + mysql.escape(plugin_name) + ", 'injected', " + mysql.escape(autostart)+", " + mysql.escape(on_maintenance) + ")", function (err, result) {
        if (err) {
...
db_utils.prototype.updatePlugin = function (plugin_id, name, version, tag_id, code, parameters, description, iotronic_dep, callback) {
   ...
   //     "type_id='" + type_id + "', " +
        ...
        "iotronic_dep=" + mysql.escape(iotronic_dep) + ", " +
        "updated_at=NOW()" +
