                                            ...
                                            if(wrong_conn_boards.length !=0){
                                                ...
                                                for (var w = 0; w < wrong_conn_boards.length; w++) {
                                                    ...
                                                    (function (w) {
                                                        ...
                                                        if(vnet_enabled) net_utils.activateBoardNetwork(wrong_conn_boards[w], false, true);
...
var onBoardConnected = function (args) {
    ...
    if (wamp_conn_status == 'connection') {
        ...
        auth.checkAuthorization(board_id, auth_credentials, function (auth_data) {
                ...
                iotronic_session.call('s4t.' + wamp_session + '.board.checkRegistrationStatus', [auth_data]).then(
                    ...
                    function (msg) {
                        ...
                        if(msg.result == "SUCCESS"){
                            ...
                            var lr_version = msg.lr_version;

                            if(confStatus.message.state == "new" || confStatus.message.state == "updated")
                                var iotronic_state = "operative"; //"registered";
                            else
                                var iotronic_state = confStatus.message.state;


                            db.changeBoardStatus(board_id, wamp_session, 'C', iotronic_state, lr_version, function (data) {
                                ...
                                if (data.result == "ERROR") {
                                    ...
                                    db.getBoard(board_id, function (data) {
                                        ...
                                        if (data.result == "ERROR") {
                                            ...
                                            //Input parameters: board_id - res = false - restore = false
                                            if(vnet_enabled) net_utils.activateBoardNetwork(board_id, false, "false");
...
//This function decides to inject the configuration to the board related to registration-status
var computeStatus = function (board_id, state, callback) {
    ...
    else if(state == "registered"){  //RETRO-COMP (same of "operative")
