...
board_utils = function (session, rest) {
            ...
            if (available.result != "ERROR") {
                ...
                checkRequired(ApiRequired, function (check){
                    ...
                    if(check.result == "ERROR"){
                        ...
                        if (state != "new" && state != "registered" && state != "maintenance"){


                            response.message = "Registration status '" + state + "' is not supported: [ 'new' | 'registered' | 'maintenance']";
                            response.result = "ERROR";
                            logger.error("[SYSTEM] --> " + response.message);
                            res.status(500).send(response);
     ...
     *                  state:
     *                      type: String
     *                      description: "Iotronic board state: 'new', registered', 'maintenance'"
     *     produces:
     *       - application/json
     *     responses:
     *       200:
    ...
    rest.patch('/v1/boards/:board/state', function (req, res) {
        ...
        STATES = ['new', 'registered', 'maintenance', "post-maintenance"];
        ...
        logger.info("[SYSTEM] - BOARD '" + board + "' state setting to '"+state+"'...");
                ...
                if(STATES.includes(state)){
                    ...
                    db.BoardInfo(board, function (data) {
                        ...
                        if (data.result == "ERROR") {
                            ...
                            old_state = data.message.info.state;

                            if(state == "registered" && old_state == "maintenance"){

                                new_lr_state = "post-maintenance";

                                logger.info("[SYSTEM] --> PRESTATE: " + old_state + " - NEW STATE: " + state);
