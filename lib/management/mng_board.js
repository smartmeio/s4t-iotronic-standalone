...
board_utils = function (session, rest) {
     ...
     *                          conn_time:
     *                              type: string
     *                              description: "timestamp of the latest connection/disconnection event"
     *                          state_time:
     *                              type: string
     *                              description: "timestamp of the latest change state event"
     *                          layout_id:
     *                              type: integer
     ...
     *                      vnets:
     *                          type: array
     *                          description: "List of vnets where the board is linked"
     *                          items:
     *                              title: vnet information
     *                              type: object
     *                      connectivity:
     *                          type: array
     *                          description: "List of connections used by the board to connect to Internet/Iotronic: each board has one main connection."
     *                          items:
     ...
     *                      description: "List of connections used by the board to connect to Internet/Iotronic: each board has one main connection.
     *                          \n e.g.:
     *                          \n [{'type':1, 'main': 'false', 'metadata':{'MAC':'1234'} }, {'type':2, 'main': 'false', 'metadata':{'MAC':'1234'} }, {'type':3, 'main': 'true', 'metadata':{'ICCID':'1234'} }]'
     *                          \n ---
     *                          \n Meaning of the 'type' field and its metadata:
     *                          \n - '1': wifi -> 'metadata':{'MAC':'1234'}
     *                          \n - '2': eth -> 'metadata':{'MAC':'1234'}
     *                          \n - '3': mobile -> 'metadata':{'ICCID':'1234'} "
     *
     *     responses:
     *       200:
    ...
    rest.post('/v1/boards/', function (req, res) {
        ...
        //var connectivity = JSON.parse(req.body.connectivity);
                    ...
                    try {
                        ...
                        auth.computeCredentials(b_pw, b_pub_key, function(credentials) {
                            ...
                            if (credentials.result == "ERROR") {
                                ...
                                db.regBoard(board_id, board_label, latitude, longitude, altitude, net_enabled, layout_id, description, connectivity,
            ...
            if (available.result != "ERROR") {
                ...
                checkRequired(ApiRequired, function (check){
                    ...
                    if(check.result == "ERROR"){
                        ...
                        if (state != "new" && state != "registered" && state != "maintenance" && state != "operative"){


                            response.message = "Registration status '" + state + "' is not supported: [ 'new' | 'registered' | 'maintenance' | 'operative']";
                            response.result = "ERROR";
                            logger.error("[SYSTEM] --> " + response.message);
                            res.status(500).send(response);
                            ...
                            auth.updateCredentials(b_pw, b_pub_key, function(credentials) {
                                ...
                                if (credentials.result == "ERROR") {
                                    ...
                                    db.updateBoard(board, board_label, latitude, longitude, altitude, net_enabled, layout_id, description, connectivity,
                                                    ...
                                                    if (check_result.result == "ERROR") {
                                                        ...
                                                        if (status === "C") {
                                                                    ...
                                                                    session_wamp.call('s4t.'+ board + '.board.updateConf', [conf]).then(
                                                                        ...
                                                                        function (rpc_response) {
                                                                            ...
                                                                            if(rpc_response.result == "ERROR"){
                                                                                ...
                                                                                response.message = "Configuration injected into the board '" + board + "': LR restarting...";
                                                            ...
                                                            */
                                                            ...
                                                            response.message = db_result.message + " - !!! WARNING: REMOTE DEVICE CONFIGURATION NOT UPDATED !!!";
                                                            response.result = "WARNING";
    ...
    rest.delete('/v1/boards/:board', function (req, res) {
        ...
        db.unRegBoard(board, function (data) {
            ...
            if (data.result == "ERROR") {
                ...
                logger.info("[SYSTEM] --> " + data.message);
                res.status(200).send(data);
            }
     ...
     *     tags:
     *       - Boards
     *     description: It returns the sensors status for each Iotronic board.
     *     summary: get Iotronic board sensors status
     *     produces:
     *       - application/json
     *     responses:
     *       200:
     *          description: Iotronic board sensors status
     *          properties:
     *              result:
     *                  type: string
     *                  description: "SUCCESS"
     *              message:
     *                  type: array
     *                  description: "Iotronic board sensors status metadata \n
     *                      \n      {
     *                      \n          'id': 1,
     *                      \n          'board_id': '1234-56-78',
     *                      \n          'timestamp': '2019-02-26T08:47:01.000Z',
     *                      \n          'report': '{ <JSON-CUSTOM-SENSORS-STATUS-REPORT> }',
     *                      \n          's_tot': '<NUMBER-OF-WORKING-SENSORS>',
     *                      \n          's_work': '<NUMBER-OF-SENSORS-OF-THE-BOARD>'
     *                      \n      } "
     *       403:
     *         description: "Wrong, expired or not specified token in request header."
     *       500:
    ...
    rest.get('/v1/sensors/', function (req, res) {
        ...
        db.getSensorStatus(function (data) {

            logger.info("[SYSTEM] - Sensors status called.");
            ...
            if (data.result == "ERROR") {
                response.message = "Error getting sensors status: " + data.message;
                response.result = "ERROR";
                logger.error("[SYSTEM] --> " + response.message);
                res.status(500).send(response);
    ...
    //Update board extra info
    /**
     * @swagger
     * /v1/boards/{board}/extra:
     *   patch:
     *     tags:
     *       - Boards
     *     description: Update board extra info
     *     summary: update IoTronic board extra info
     *     parameters:
     *      - in: path
     *        name: board
     *        required: true
     ...
     *         description: "API specific error message."
     */
    rest.patch('/v1/boards/:board/extra', function (req, res) {
        ...
        logger.info("[SYSTEM] - BOARD '" + board + "' extra info updating...");
        ...
        checkRequired(ApiRequired, function (check){
            ...
            if(check.result == "ERROR"){
                ...
                db.updateDeviceExtraInfo(board, extra, function (db_result) {
                        ...
                        if (db_result.result == "ERROR") {
                            ...
                            logger.info("[SYSTEM] --> " + db_result.message);
                            response.message = db_result.message;
                            response.result = "SUCCESS";
                            res.status(200).send(response);
    ...
    /**
     * @swagger
     * /v1/boards/{board}/state:
     *   patch:
     *     tags:
     *       - Boards
     *     description: Update board state
     *     summary: update IoTronic board state
     *     parameters:
     *      - in: path
     *        name: board
     *        required: true
     ...
     *                  state:
     *                      type: String
     *                      description: "Iotronic board state: 'new', 'operative', 'maintenance'"
     *     produces:
     *       - application/json
     *     responses:
     *       200:
     ...
     *         description: "API specific error message."
     */
    rest.patch('/v1/boards/:board/state', function (req, res) {

        logger.info("[API] - Change device state - " + Object.keys( req.route.methods ) + " - " + req.route.path + " - " + req.IotronicUser);

        var board = req.params.board;
        var state = req.body.state;

        STATES = ['new', 'registered', 'maintenance', 'operative'];
        ...
        checkRequired(ApiRequired, function (check){
            ...
            if(check.result == "ERROR"){
                ...
                var request_id = null;
                board_utils.changeState(board, request_id, [state, res]);
                ...
                if(STATES.includes(state)){
                    ...
                    // Get old state saved in Iotronic DB
                    db.BoardInfo(board, function (data) {
                        ...
                        if (data.result == "ERROR") {
                            ...
                            db.changeBoardState(board, new_lr_state, function (db_result) {
                                    ...
                                    db.checkBoardConnected(board, function (check_result) {
                                        ...
                                        if (check_result.result == "ERROR") {
                                            ...
                                            if (status === "C") {
                                                ...
                                                session_wamp.call('s4t.' + board + '.board.changeState', [new_lr_state]).then(

                                                    function (result) {
                                                        response.message = result.message;
                                                        response.result = "SUCCESS";
                                                        logger.info("[SYSTEM] --> " + JSON.stringify(response.message));
                                                        res.status(200).send(response);
                                                    },
                                                    function (rpc_error) {
                                                ...
                                                response.message = "device state will change to '" + state + "' at next reconnection.";
                                                response.result = "SUCCESS";
                                                logger.info("[SYSTEM] --> " + response.message);
                                                res.status(200).send(response);
                                            }
                    ...
                    response.message = "board state unsupported!";
                    response.result = "ERROR";
                    logger.error("[SYSTEM] --> " + response.message);
     ...
     *     tags:
     *       - Boards
     *     summary: change state for each device of a project
     *     description: Change state for each device of a project
     *     produces:
     *       - application/json
     *     parameters:
     *      - in: path
     ...
     *         description: "API specific error message."
     */
    rest.patch('/v1/projects/:project/boards/state', function (req, res) {
        ...
        var subject = "change to '"+state+"' state";
        var rest = false;

        request_utils.batchRequest(project, subject, res, board_utils.changeState, [state, rest]);
    ...
    /**
     * @swagger
     * /v1/boards/{board}/conn_status:
     *   patch:
     *     tags:
     *       - Boards
     *     description: Force connection status
     *     summary: force device connection status
     *     parameters:
     *      - in: path
     *        name: board
     *        required: true
     ...
     *                  status:
     *                      type: String
     *                      description: "Device connection status ['C' | 'D']"
     *     produces:
     *       - application/json
     *     responses:
     *       200:
     ...
     *         description: "API specific error message."
     */
    rest.patch('/v1/boards/:board/conn_status', function (req, res) {
        ...
        logger.info("[SYSTEM] - BOARD '" + board + "' connection status forcing to '"+status+"'...");
        ...
        checkRequired(ApiRequired, function (check){
            ...
            if(check.result == "ERROR"){
                ...
                if(STATUS.includes(status)){
                    ...
                    db.forceConnStatus(board, status, function (db_result) {
                    ...
                    response.message = "connection status unsupported: only [ C | D ]";
                    response.result = "ERROR";
                    logger.error("[SYSTEM] --> " + response.message);
            ...
            session_wamp.call('s4t.' + board_id + '.board.updateLR', [lr_version, pkg_mng, operation]).then(
                ...
                function (rpc_response) {
                    ...
                    if (rpc_response.result == "ERROR") {
                        ...
                        db.changeBoardStateOnUpdate(board_id, "updated", lr_version, function (response) {
        ...
        if (data.result == "ERROR") {
            ...
            if (data.message.length == 1) {
                ...
                if (data.message[0].status == 'D') {
                    ...
                    response.state = data.message[0].state;
                    callback(response);
...
var createBoardConf = function (board_id, callback) {
    ...
    db.BoardInfo(board_id, function (data) {
        ...
        if (data.result == "ERROR") {
            ...
            settings.config.extra = data.message.info.extra;
...
board_utils.prototype.changeState = function (board_id, request_id, args){
    ...
    logger.info("[SYSTEM] - Device '" + board_id + "' state setting to '"+state+"'...");
    ...
    if(STATES.includes(state)){
        ...
        db.BoardInfo(board_id, function (data) {
            ...
            if (data.result == "ERROR") {
                ...
                db.changeBoardState(board_id, new_lr_state, function (db_result) {
                    ...
                    if (db_result.result == "ERROR") {
                        ...
                        db.checkBoardConnected(board_id, function (check_result) {

                            if (check_result.result == "ERROR") {

                                response.result = check_result.result;
                                response.message = "DB checkBoardConnected error for board " + board_id + ": " + check_result.message;
                                ...
                                if (status === "C") {
                                    ...
                                    session_wamp.call('s4t.' + board_id + '.board.changeState', [new_lr_state]).then(
                                        ...
                                        function (result) {
                                            ...
                                            if(res != false){
                                                logger.info("[SYSTEM] --> LR: " + JSON.stringify(response.message));
                                                res.status(200).send(response);
                                            }
                                            else
                                                request_utils.updateResult(request_id, board_id, response.result, response.message);
                                    ...
                                    response.message = "device '"+board_id+"' state will change to '" + state + "' at next reconnection.";
                                    response.result = "SUCCESS";
